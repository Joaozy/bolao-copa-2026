...
export async function GET() {
  ...
  try {
    ...
    // Filtra no JavaScript para precisão de fuso horário
    const jogosProximos = todosJogos?.filter(jogo => {
      const jogoData = new Date(jogo.start_time)
      const diffMinutos = (jogoData.getTime() - agora.getTime()) / 60000
      return diffMinutos > 0 && diffMinutos <= MINUTOS_ANTECEDENCIA
    }) || []

    if (jogosProximos.length === 0) {
      return NextResponse.json({ message: "Nenhum jogo na janela de 30min." })
    }
        ...
        if (zapiInstanceId && zapiToken) {
            try {
                ...
                if (response.ok) {
                    ...
                    erroDetalhado = await response.json() 
                }
            } catch (err) {
