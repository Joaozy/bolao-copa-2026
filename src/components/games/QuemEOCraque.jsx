...
export default function QuemEOCraque() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const jogadoresValidos = JOGADORES_COPA.filter(p => p.overall >= 74).map(p => ({
        ...p, team_name: timesMap[p.team_id]?.name || 'Desconhecido', badge_url: timesMap[p.team_id]?.badge_url || null
      }));

      setTodosJogadores(jogadoresValidos);
      escolherResposta(jogadoresValidos);
      setCarregando(false);
    }
    load();
  }, []);

  const escolherResposta = (lista) => {
  ...
  if (carregando) return <div style={{ minHeight:'100vh', background:'#08111f', display:'flex', alignItems:'center', justifyContent:'center', color:'#f2c14e' }}>Carregando Craques...</div>;

  return (
    <div style={{ minHeight:'100vh', background:'#08111f', color:'#f4f1ea', fontFamily:"'Inter', sans-serif" }}>
      {/* O CSS PODE MANTER EXATAMENTE O MESMO QUE VOCÊ JÁ TEM NO ARQUIVO ANTIGO */}
      <style>{`
        ...
        *{box-sizing:border-box;} .crq-wrap{max-width:760px;margin:0 auto;padding:28px 16px 64px;}
        ...
        .crq-sub{text-align:center;color:rgba(244,241,234,.5);font-size:14px;margin:6px 0 0; font-family:'JetBrains Mono',monospace;}
        ...
        .crq-input{width:100%;background:#0d1a2e;border:2px solid rgba(242,193,78,.35); border-radius:10px;padding:13px 16px 13px 16px;font-size:15px;color:#f4f1ea;outline:none;}
        ...
        .crq-sug-flag{width:24px;border-radius:3px;}
      ...
      <div className="crq-wrap">
        ...
        <p className="crq-sub">{status === 'jogando' ? `${MAX_TENTATIVAS} tentativas restantes` : status === 'ganhou' ? '⚽ Acertou!' : '💀 Fim de jogo'}</p>
              ...
              <div className="crq-dropdown">
                {sugestoes.map(j => (
                  <div key={j.id} className="crq-sug" onClick={() => confirmarPalpite(j)}>
                    {j.badge_url && <img src={j.badge_url} alt="" className="crq-sug-flag" />}
                    <span style={{flex:1}}>{j.name}</span>
                    <span style={{fontSize:10, color:'rgba(244,241,234,.4)'}}>{j.pos1} · OVR {j.overall}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {(status === 'ganhou' || status === 'perdeu') && resposta && (
          <div className="crq-result" style={{ background: status === 'ganhou' ? 'rgba(34,197,94,.1)' : 'rgba(215,38,61,.1)' }}>
            ...
            <button className="crq-btn" onClick={() => escolherResposta()}>🎲 Jogar Novamente</button>
          </div>
        )}
      </div>
    </div>
